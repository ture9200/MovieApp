...
function LandingPage() {
    ...
    const fetchMovies = (endpoint) => { /* 로드되자마자 endpoint 이용한다. */ 
        fetch(endpoint)
            ...
            .then(response => { /* 그 결과값을 다시 response로 가져오면 20개 가장 유명한 영화가 보이는데 이름, overview, poster image등을 보여주게 된다. */ 
                ...
                /* ... response.results만 하면 처음 20개는 사라지고 다음 20개가 덮어진다. 덮어씌우는게 아니라 원래 있던거에 추가를 하려면 movie의 state 를 넣어주면 된다. */ 
                setMainMovieImage(response.results[0]) /* response에 있는 첫번째 result를 setMainMovieImage에 넣어준다 */ 
    ...
    /* 버튼을 누를때마다 다음 20개가 로딩 될 수 있도록한다 */ 
        ...
        currentpage  가 처음엔 0이니까 0일때는 1, 1일때는 2가 되고 2일때는 3 이 된다 */ 
        fetchMovies(endpoint)
        ...
        <div style={{ width: '100%', margin: '0' }}>
            ...
            <div style={{ width: '85%', margin: '1rem auto' }}>
                ...
                <Row gutter={[16, 16]} > 
                    ...
                    {Movies && Movies.map((movie, index) => (  /* movies가 있으면 map메서드를 이용해서 하나하나 컨트롤 할 수 있게 하나의  무비들을 가져온다 
                        ...
                        <React.Fragment key={index}> 
                            <GridCards      /* prop으로 grid card 의 정보들을 뿌려줘야한다. prop을 넣어준다. */  
                                ...
                                image={movie.poster_path ?
                                    ...
                                    poster path 가 있는 영화면 image base url 을 넣어준다.
